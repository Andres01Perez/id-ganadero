...
export const agentClientTools = {
  ...
  detalle_animal: async (params: ToolParams = {}) => {
    ...
    const { data, error } = await query;
    if (error) return fail(error.message);
    const animal = data?.[0] as
      | (AnimalSummary & { numero_registro?: string | null; madre_id?: string | null; padre_id?: string | null })
      | undefined;
  ...
  resumen_ganaderia: async (params: ToolParams = {}) => {
    ...
    const porTipo = (animales ?? []).reduce<Record<string, number>>((acc, a) => {
      acc[a.tipo] = (acc[a.tipo] ?? 0) + 1;
      return acc;
    }, {});
    const porSexo = (animales ?? []).reduce<Record<string, number>>((acc, a) => {
      const s = a.sexo === "M" ? "machos" : a.sexo === "H" ? "hembras" : "sin_sexo";
      acc[s] = (acc[s] ?? 0) + 1;
      return acc;
    }, {});
    const fincaNames = new Map((fincas ?? []).map((f) => [f.id, f.nombre]));
    const porFinca = (animales ?? []).reduce<Record<string, number>>((acc, a) => {
      const name = fincaNames.get(a.finca_id) ?? "Sin finca";
    ...
    return result({
      ...
      finca: fincaNombres.length ? fincaNombres : null,
      por_tipo: porTipo,
      por_sexo: porSexo,
      por_finca: porFinca,
    });
